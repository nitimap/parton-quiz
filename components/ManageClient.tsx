...
export function ManageClient() {
  ...
  return <main className="shell py-10 sm:py-14">
    ...
    <section className="mt-10"><div><p className="eyebrow">Progress tracking</p><h2 className="mt-2 text-2xl font-bold">Assignments & Recent Activity</h2></div><div className="mt-4 grid gap-4">{assignments.map(a=>{const attempts=[...(a.attempts??[])].sort((x,y)=>new Date(y.started_at).getTime()-new Date(x.started_at).getTime()),latest=attempts[0];return <article className="card p-5" key={a.id}><div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="text-lg font-bold">{a.label}</h3><p className="text-sm text-[var(--muted)]">{a.quizzes?.title} · {a.quizzes?.subject||"No subject"}</p></div><button className="btn btn-secondary" onClick={()=>navigator.clipboard.writeText(`${window.location.origin}/quiz/${a.quiz_id}?assignment=${a.token}`)}>Copy Link</button></div><div className="mt-4 rounded-xl bg-[var(--blue-soft)] p-4">{!latest?<strong>Not started</strong>:latest.status==="in_progress"?<><strong>In progress</strong><span className="ml-2 text-sm text-[var(--muted)]">Started {new Date(latest.started_at).toLocaleString()}</span></>:<><strong className="text-[var(--green)]">Completed · {latest.score_percent}%</strong><span className="ml-2 text-sm text-[var(--muted)]">{latest.correct_count}/{latest.question_count} correct · {new Date(latest.completed_at??latest.started_at).toLocaleString()}</span></>}</div>{attempts.length>1&&<details className="mt-3"><summary className="cursor-pointer text-sm font-bold text-[var(--blue)]">View all {attempts.length} attempts</summary><ul className="mt-2 space-y-1 text-sm">{attempts.map(item=><li key={item.id}>{item.status==="completed"?`${item.score_percent}% (${item.correct_count}/${item.question_count})`:"In progress"} · {new Date(item.started_at).toLocaleString()}</li>)}</ul></details>}</article>})}{!assignments.length&&<p className="card p-6 text-[var(--muted)]">No assignments yet. Create a link from a quiz above and send it to your child.</p>}</div></section>
  </main>;
}
